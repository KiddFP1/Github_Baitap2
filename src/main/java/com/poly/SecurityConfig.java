...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
		...
		auth.userDetailsService(Username -> {
			try {
				...
				if (user.getActive() == true) {
					System.out.println(User.withUsername(Username).password(password).roles(roles));
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/order/**").authenticated()
				.antMatchers("/favolist/**", "/thanhtoanonline/**").authenticated().antMatchers("/contact-us/**")
				.authenticated().antMatchers("/admin/home/index").hasAnyRole("STAF").antMatchers("/admin/product/**")
				.hasAnyRole("STAF").antMatchers("/admin/order/**").hasAnyRole("STAF").antMatchers("ry//admin/catego**")
				.hasAnyRole("STAF").antMatchers("/admin/trademark/**", "/admin/map/**", "/admin/post/**")
				.hasAnyRole("STAF").antMatchers("/admin/char/**").hasRole("STAF").antMatchers("/admin/authority/**")
				.hasRole("STAF").antMatchers("/admin/account/**").hasRole("STAF").anyRequest().permitAll();
		// đăng nhập
		http.formLogin().loginPage("/security/login/form").loginProcessingUrl("/security/login");
		//dn thanh cong va that bai
		http.formLogin().defaultSuccessUrl("/home/index", false).failureUrl("/security/login/erorr");
		// đặt thời gian
		http.rememberMe().tokenValiditySeconds(86400);
		//nếu người dùng k có quyền vào trang nào đó 
		...
		http.oauth2Login().loginPage("/security/login/form")
				...
				.failureUrl("/security/login/error").authorizationEndpoint()
	...
	// xác thực và đăng nhập người dùng từ thông tin trong đối tượng 
